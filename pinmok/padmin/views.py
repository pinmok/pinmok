...
from pinmok.core import api
from pinmok.core.api import ErrorCode
from pinmok.core.libs.sprite import SpriteManager, SpriteError
from pinmok.core.mixins import PinmokPermissionMixin
from pinmok.padmin.constants import CUSTOM_SPRITE_FILE, PINMOK_ICON_PREFIX, PINMOK_SPRITE_FILE
from pinmok.padmin.enums import FileType, ConfigCategory
from pinmok.padmin.models import UrlAlias
...
from pinmok.padmin.service.menu import MenuSyncMode
from pinmok.padmin.service.navigation import NavService
from pinmok.padmin.service.theme import ThemeService, ThemeServiceError
...
class SpriteManagerView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if action == 'edit':
            if not symbol_id:
                ...
                return redirect(reverse('admin:padmin:icons_manage'))
        ...
        try:
            system_sprite_path = finders.find(PINMOK_SPRITE_FILE)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if symbol_id.startswith(PINMOK_ICON_PREFIX):
            messages.error(request, _("Symbol ID cannot start with '%(prefix)s'.") % {'prefix': PINMOK_ICON_PREFIX})
            return TemplateResponse(request, self.add_template_name, context)
...
class UploadFileView(PinmokPermissionMixin, View):
    """
...
class ThemeView:
    ...
    def install(request, directory: str):
        ...
        return redirect(reverse('admin:padmin:theme_list'))
