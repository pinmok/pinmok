...
Description:
  Template tags and filters for the Pinmok interface.
Author:
  惠达浪 <crazys@126.com>
Created:
  2026/4/25
...
from pinmok.padmin.models import ExternalLink, Slider
...
def icon(icon_name: str, css_class: str = '', size: int = 24) -> str:
    ...
    - Names starting with 'tabler-' use the built-in Pinmok sprite.
    ...
    if icon_name.startswith(PINMOK_ICON_PREFIX):
        sprite_path = static(PINMOK_SPRITE_FILE)
    else:
...
def external_links() -> list[dict[str, Any]]:
    ...
    if links is None:
        ...
        cache.set(EXTERNAL_LINK_CACHE_KEY, links, PINMOK_CONFIG_CACHE_TTL)
    return links
...
def slider(group: str) -> list[dict[str, Any]]:
    ...
    if items is None:
        ...
        cache.set(cache_key, items, PINMOK_CONFIG_CACHE_TTL)
    return items
