...
Description:
  ...
  Each ConfigType maps to a specific Django form field and Pinmok widget.
  ...
  Example::
    ...
    class SiteConfigAdmin(PinmokModelAdmin):
        form = SiteConfigForm
        fieldsets = [
...
from pinmok.padmin.config_schema import CONFIG_SCHEMA
from pinmok.padmin.enums import ConfigType, ConfigCategory, UploadConfigKey
from pinmok.padmin.service.config import ConfigService
from pinmok.padmin.widgets import PinmokCheckboxSelectMultiple
...
_FIELD_MAP: dict[ConfigType, tuple[type[forms.Field], type[forms.Widget] | None]] = {
    ConfigType.STR: (forms.CharField, widgets.PinmokTextInput),
    ConfigType.TEXT: (forms.CharField, widgets.PinmokTextarea),
    ConfigType.INT: (forms.IntegerField, widgets.PinmokNumberInput),
    ConfigType.FLOAT: (forms.FloatField, widgets.PinmokNumberInput),
    ConfigType.BOOL: (forms.BooleanField, widgets.PinmokCheckbox),
    ConfigType.JSON: (forms.CharField, widgets.PinmokTextarea),
    ConfigType.DATETIME: (forms.DateTimeField, widgets.PinmokDateTimeInput),
    ConfigType.IP: (forms.GenericIPAddressField, widgets.PinmokGenericIPAddress),
    ConfigType.URL: (forms.URLField, widgets.PinmokURLInput),
    ConfigType.EMAIL: (forms.EmailField, widgets.PinmokEmailInput),
    ConfigType.IMAGE: (forms.CharField, widgets.PinmokImageFileInput),
    ConfigType.FILE: (forms.FileField, widgets.PinmokFileInput),
    ConfigType.MULTI_SELECT: (forms.MultipleChoiceField, widgets.PinmokSelectTags),
}
...
def _build_field(key: str, field_schema: dict) -> forms.Field:
    ...
    field_class, widget_class = _FIELD_MAP.get(cfg_type, (forms.CharField, widgets.PinmokTextInput))
    ...
    if widget is None:
        if choices and cfg_type == ConfigType.MULTI_SELECT:
            ...
            # Single-select: use PinmokSelect
...
class UploadConfigForm(ConfigForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key in [
            ...
            self.fields[key].widget = PinmokCheckboxSelectMultiple(
