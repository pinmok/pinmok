...
Description:
  ...
  This module defines ModelAdmin classes and registers Pinmok models
  using the @padmin.register() decorator.
Author:
  惠达浪 <crazys@126.com>
Created:
  2025-06-08
...
from pinmok.padmin.constants import EXTERNAL_LINK_CACHE_KEY
from pinmok.padmin.enums import ConfigCategory, UploadConfigKey, FileType, MimeType
from pinmok.padmin.fields import IndentedModelChoiceField
from pinmok.padmin.forms.config_forms import EmailConfigForm, UploadConfigForm
from pinmok.padmin.forms.forms import PinmokAdminPasswordResetForm, PinmokAdminUserCreationForm
from pinmok.padmin.models import ExternalLink, SiteConfig, EmailConfig, UploadConfig, Resource, Nav, NavTranslation, UrlAlias, Slider
from pinmok.padmin.options import PinmokModelAdmin, PinmokModelAdminMixin, ConfigModelAdmin, ExtraPanel, PinmokStackedInline
from pinmok.padmin.service.navigation import NavService
from pinmok.padmin.service.upload import UploadService


@padmin.register(User)
class PinmokUserAdmin(PinmokModelAdminMixin, UserAdmin):
    """
    Custom admin for Django's User model with Pinmok enhancements.
    """
    fieldsets = (
    ...
    add_form = PinmokAdminUserCreationForm
    change_password_form = PinmokAdminPasswordResetForm


@padmin.register(Group)
class PinmokGroupAdmin(PinmokModelAdminMixin, GroupAdmin):
    """
    Custom admin for Django's Group model with Pinmok enhancements.
    """
    pass
...
@padmin.register(Resource)
class ResourceAdmin(PinmokModelAdmin):
    """
...
class NavTranslationInline(PinmokStackedInline):
...
class NavAdmin(PinmokModelAdmin):
    ...
    def formfield_for_dbfield(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'group' and field is not None:
            ...
            field.widget = widgets.PinmokDatalistInput(options=options)
            field.widget.attrs['data-parent-choices-url'] = reverse('admin:padmin:nav_parent_choices')
        return field
