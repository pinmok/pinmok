...
??? abstract "Quick Start"
    ...
    **Enable Django i18n**
    ...
    Set `USE_I18N = True` in `settings.py` and add `LocaleMiddleware` to `MIDDLEWARE` after
    `SessionMiddleware`. The `padmin` interface language will follow the active Django locale
...
Pinmok's multilingual support is built entirely on Django's i18n framework — no extra
configuration layer is introduced. `padmin` ships with built-in translations for Simplified
Chinese (`zh-hans`) and English (`en`), and the interface language follows the active Django
locale automatically.

Beyond standard Django i18n, Pinmok provides two supplementary tools that address scenarios
Django's built-in solution doesn't cover: **multilingual model base classes**
(`TranslatableModel` / `TranslationModel`) for storing and retrieving multilingual database
content, and the **PO deduplication tool** (`dedupe_po`) for removing redundant entries from
`.po` files.
