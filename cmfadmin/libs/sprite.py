...
class SpriteManager:
    ...
    Methods:
        ...
        has(symbol_id) -> bool
        list() -> dict
        response(success, error_code, msg, data)
    """
    ...
    def __init__(self, sprite_file: str, viewbox: str = '0 0 24 24'):
        ...
        try:
            ...
            raise FileNotFoundError(f"Sprite file '{self.sprite_file}' not found")
        except ET.ParseError:
            raise ValueError(f"Sprite file '{self.sprite_file}' format error")

    @staticmethod
    def response(success: bool, error_code='OK', msg: str = '', data: dict = None) -> dict:
        return {
            'success': success,
    ...
    def create(self):
        ...
        if os.path.exists(self.sprite_file):
            return self.response(False, 'FILE_EXISTS', _('File already exists'))

        try:
            with open(self.sprite_file, 'w', encoding='utf-8') as f:
                f.write('<svg xmlns="http://www.w3.org/2000/svg" style="display:none">\n</svg>')
            return self.response(True, 'OK', _('File created successfully'))
        except Exception as e:
            return self.response(False, 'CREATE_FAILED', str(e))
    ...
    def list(self) -> dict:
        """List all <symbol> IDs."""
        symbols = [e.attrib.get('id') for e in self.root.findall('svg:symbol', self.SVG_NS) if e.attrib.get('id')]
        return self.response(
            True,
            'OK',
            _('Fetched symbol list successfully'),
